<x-app-layout>
    <x-slot name="header">
        <div id="crud_title">{{ __('Editar ') . $unit_group->getTranslatedType() }}</div>
    </x-slot>

    <div class="overflow-x-auto bg-white border border-gray-300 rounded-lg shadow-lg">
        <form action="{{ route('products.unit_groups.update', ['product' => $product->id, 'unit_group' => $unit_group->id]) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6 sm:col-span-1">
                            ...
                            <select name="type" id="type" placeholder="{{ __('...') }}"
                                ...
                                <option value="block" {{ (old('type') ?? $unit_group->type) == 'block' ? 'selected' : '' }}>Bloco</option>
                                <option value="tower" {{ (old('type') ?? $unit_group->type) == 'tower' ? 'selected' : '' }}>Torre</option>
                                <option value="village" {{ (old('type') ?? $unit_group->type) == 'village' ? 'selected' : '' }}>Vila</option>
                                <option value="square" {{ (old('type') ?? $unit_group->type) == 'square' ? 'selected' : '' }}>Quadra</option>
                            </select>
                            @error('type')
                        ...
                        <div class="col-span-6 sm:col-span-1">
                            ...
                            <input type="text" name="number" id="number" autocomplete="given-number"
                                ...
                                value="{{ old('number') ?? $unit_group->number }}">
                            @error('number')
                                <small class="text-red-700 font-sm">*{{ $message }}</small>
                            @enderror
                        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', (e) => {
                $('#type').on('change', function() {
                    var content = "{{ __('Editar ') }}" + $("#type option:selected").text();
