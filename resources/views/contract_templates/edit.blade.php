<x-app-layout>
    ...
    <div class="overflow-x-auto bg-white border border-gray-300 rounded-lg shadow-lg">
        <form action="{{ route('contract_templates.update', $template->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6 sm:col-span-2">
                            ...
                            <input type="text" name="name" id="name" autocomplete="given-name"
                                ...
                                value="{{ old('name') ?? $template->name }}" autofocus>
                            @error('name')
                        ...
                        <div class="col-span-6 md:col-span-1">
                            ...
                            <select id="active" name="active"
                                ...
                                <option @if (old('active') ?? $template->active) selected @endif value="true" )>Ativa</option>
                                <option @if (!old('active') ?? $template->active) selected @endif value="false" )>Inativa</option>
                            </select>
                            @error('active')
                                <small class="text-red-700 font-sm">*{{ $message }}</small>
                            @enderror
                        ...
                        <div class="col-span-6 md:col-span-2">
                            ...
                            <select id="product_id" name="product_id"
                                ...
                                @foreach($products as $product)
                                    <option @if ((old('product_id') ?? $template->product_id) == $product->id) selected @endif value="{{ $product->id }}" )>{{ $product->name }}</option>
                                @endforeach
                            </select>
                            @error('product_id')
                                <small class="text-red-700 font-sm">*{{ $message }}</small>
