<x-app-layout>
    ...
    <div class="px-6 py-4 mt-2 mb-4 bg-white border border-gray-300 rounded-lg shadow-lg">
        <div class="flex flex-col items-center justify-between md:flex-row">
            ...
            <a href="{{ route('contract_templates.create') }}"
                    ...
                    <tr class="bg-white">
                        ...
                        <td class="px-6 py-4 text-sm font-bold rounded-b-lg whitespace-nowrap">
                            <div class="flex justify-end gap-4">
                                <a href="{{ route('contract_templates.edit', $template->id) }}"
                                    title="{{ __('Editar') }}"
                                ...
                                <form action="{{ route('contract_templates.destroy', $template->id) }}" method="POST"
                                    class="delete-form" data-id="{{ $template->id }}"
                                    data-description="{{ $template->name }}"
                                    data-msg="{{ __('Deseja remover a minuta ') }}">
                                    @csrf
                                    @method('DELETE')
