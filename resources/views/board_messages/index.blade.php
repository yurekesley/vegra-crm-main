<x-app-layout>
    ...
    <div class="px-6 py-4 mt-2 mb-4 bg-white border border-gray-300 rounded-lg shadow-lg">
        <div class="flex flex-col items-center justify-between md:flex-row">
            ...
            <a href="{{ route('board_messages.create') }}"
            ...
            <tbody class="overflow-x-auto bg-white divide-y divide-gray-200">
                ...
                @foreach ($messages as $message)
                    <tr class="bg-white">
                        ...
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 rounded-b-lg whitespace-nowrap">
                            {{ $message->start_date->format('d/m/Y') }}
                        </td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 rounded-b-lg whitespace-nowrap">
                            {{ $message->end_date->format('d/m/Y') }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm font-bold rounded-b-lg whitespace-nowrap">
                            <div class="flex justify-end gap-4">
                                <a href="{{ route('board_messages.edit', $message->id) }}"
                                    title="{{ __('Editar') }}"
                                ...
                                <form action="{{ route('board_messages.destroy', $message->id) }}" method="POST"
                                    class="delete-form" data-id="{{ $message->id }}"
                                    data-description="{{ $message->name }}"
                                    data-msg="{{ __('Deseja remover a mensagem ') }}">
                                    @csrf
                                    @method('DELETE')
